...
<div class="container">
  ...
  <li role="presentation"><a href="questioncorner.php">Add question</a></li>
  <li role="presentation" class="active"><a href="questioncornersearch.php">Search question</a></li>
</ul>
    <h3 class="text-center">Search question</h3>
    <form class="form-horizontal" role="form" action="questioncornersearch.php" method="post">
        <div class="form-group">
          <label for="question" class="col-sm-1 control-label">Question</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" name="question" id="question" placeholder="Enter search question here">
          </div>
        </div>
        <div class="form-group">
	...
	if($_POST)
	{
		...
		$tag5=$_POST['tag5'];
		echo '<ul>';
		$sql="SELECT * FROM `question` WHERE ((question LIKE '%$search%') OR (tag1 LIKE '$tag1') OR (tag2 LIKE '$tag2') OR (tag3 LIKE '$tag3') OR (tag4 LIKE '$tag4') OR (tag5 LIKE '$tag5') )";
		$prime=mysqli_query($con,$sql);
		$i=0;
